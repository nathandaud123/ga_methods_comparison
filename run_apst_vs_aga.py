...
# --- Configuration ---
METHODS_TO_TEST = ['APST_GA', 'AGA']
DEFAULT_ML_METHOD = 'DT' # Using Decision Tree as default for APST based on Tahap 2 insights
...
class GeneticAlgorithm:
    def __init__(self, instance, method: str, use_gpu: bool = False):
        self.instance = instance
        self.method = method # 'APST_GA' or 'AGA'
    ...
    def mathematical_crossover_rate(self, d1, d2, d_avg, d_min):
        d_prime = max(d1, d2)
        if d_prime <= d_avg:
            if d_avg != d_min:
                try:
                    exponent = A * ((2 * (d_avg - d_prime) / (d_avg - d_min)) - 1)
                    exponent = min(700, max(-700, exponent))
                    pc = ((Pcmax - Pcmin) / (1 + exp(exponent))) + Pcmin
                except Exception:
                    pc = Pcmin
            else:
                pc = Pcmin
    ...
    def mathematical_mutation_rate(self, d, d_avg, d_min):
        if d <= d_avg:
            if d_avg != d_min:
                try:
                    exponent = A * ((2 * (d_avg - d) / (d_avg - d_min)) - 1)
                    exponent = min(700, max(-700, exponent))
                    pm = ((Pmmax - Pmmin) / (1 + exp(exponent))) + Pmmin
                except Exception:
                    pm = Pmmin
            else:
                pm = Pmmin
    ...
    def train_ml_models(self):
        ...
        # We enforce Decision Tree for the APST approach
    ...
    def predict_parameters(self, generation, favg, fmin, fmax, diversity, p1_fit, p2_fit):
        if not self.ml_trained or self.method == 'AGA':
            return None, None
    ...
    def run(self):
        ...
        for generation in range(MAX_GENERATIONS):
            d_avg, d_min, d_max = np.mean(distances), np.min(distances), np.max(distances)
            diversity = self.calculate_diversity(population)
            
            if self.method == 'APST_GA' and generation == BOOTSTRAP_GENERATIONS:
            ...
            while len(new_population) < POPULATION_SIZE:
                ...
                d1, d2 = distances[parent_indices[0]], distances[parent_indices[1]]
                
                # Determine Rates
                if self.method == 'AGA' or generation < BOOTSTRAP_GENERATIONS or not self.ml_trained:
                    pc = self.mathematical_crossover_rate(d1, d2, d_avg, d_min)
                else:
                    pc, _ = self.predict_parameters(generation, d_avg, d_min, d_max, diversity, d1, d2)
                    if pc is None:
                        pc = self.mathematical_crossover_rate(d1, d2, d_avg, d_min)
                ...
                # Mutate c1
                if self.method == 'AGA' or generation < BOOTSTRAP_GENERATIONS or not self.ml_trained:
                    pm1 = self.mathematical_mutation_rate(d1, d_avg, d_min)
                else:
                    _, pm1 = self.predict_parameters(generation, d_avg, d_min, d_max, diversity, d1, d1)
                    if pm1 is None: pm1 = self.mathematical_mutation_rate(d1, d_avg, d_min)
                c1 = self.mutation.mutate(c1, mutation_rate=pm1)
                
                # Mutate c2
                if self.method == 'AGA' or generation < BOOTSTRAP_GENERATIONS or not self.ml_trained:
                    pm2 = self.mathematical_mutation_rate(d2, d_avg, d_min)
                else:
                    _, pm2 = self.predict_parameters(generation, d_avg, d_min, d_max, diversity, d2, d2)
                    if pm2 is None: pm2 = self.mathematical_mutation_rate(d2, d_avg, d_min)
                c2 = self.mutation.mutate(c2, mutation_rate=pm2)
                
                c1 = self.representation.repair(c1)
                c2 = self.representation.repair(c2)
                
                # Collect Data only if APST_GA
                if self.method == 'APST_GA' and generation < BOOTSTRAP_GENERATIONS:
                    c1_out_d = self.representation.calculate_fitness(c1, self.dist_matrix)
                    c2_out_d = self.representation.calculate_fitness(c2, self.dist_matrix)
                    
                    # We evaluate improvement using maximization metric
                    f1 = self._to_maximization_fitness(c1_out_d)
                    f2 = self._to_maximization_fitness(c2_out_d)
                    p1_fit = self._to_maximization_fitness(d1)
                    p2_fit = self._to_maximization_fitness(d2)
                    
                    improvement1 = max(0, f1 - p1_fit)
                    improvement2 = max(0, f2 - p2_fit)
                    
                    self.training_data['features'].append([d_avg, d_min, d_max, diversity, generation/MAX_GENERATIONS, max(d1, d2), min(d1, d2)])
...
def worker_func(task):
    instance_name, dataset_type, category, method, solomon_base, uchoa_base, results_dir, use_gpu = task
    ...
    print(f"Starting {method} on {dataset_type} instance {instance_name}...")
    
    # Check if existing result found
    dataset_out_dir = os.path.join(results_dir, dataset_type, instance_name)
    out_file = os.path.join(dataset_out_dir, f"{method}_convergence.csv")
    if os.path.exists(out_file):
        print(f"Skipping {method} on {instance_name}, result already exists.")
        return
    ...
    for r in range(NUM_RUNS):
        ga = GeneticAlgorithm(instance, method, use_gpu)
        res = ga.run()
...
def main():
    parser = argparse.ArgumentParser(description="Run APST GA vs AGA (Non-APST) experiments.")
