...
OUTPUT_DIR = r"e:\TESIS\01. Dokumen\03. Publikasi Komparasi\ga_method_comparison\final_polished_v7"

if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

print("Starting FINAL POLISH V7 Analysis...")
...
# --- REVISION 1: Boxplot per Class ---
print("Generating Fig 2 V7: Boxplots...")

def plot_class_boxplot_v7(class_prefix, filename_suffix):
    # Strict filtering to prevent RC leaking into R
    if class_prefix == tuple(['R']):
        # Match R1, R2 but EXCLUDE RC1, RC2
        subset = df_full[df_full['Instance'].str.startswith('R') & ~df_full['Instance'].str.startswith('RC')].copy()
    else:
        # Standard behavior for C and RC
    ...
    plt.ylabel("Normalized Fitness Gap", fontsize=15, weight='bold') 
...
plot_class_boxplot_v7(tuple(['C']), "C")
plot_class_boxplot_v7(tuple(['R']), "R")
plot_class_boxplot_v7(tuple(['RC']), "RC")


# --- REVISION 2: 3D Plot ---
print("Generating Fig 4 V7: 3D Plots...")

def plot_3d_v7(rep_name):
    ...
    ax.set_xlabel('Selection Method', labelpad=70, fontsize=18, weight='bold')
    ax.set_ylabel('Crossover Operator', labelpad=40, fontsize=18, weight='bold')
    ax.set_zlabel('Mutation Strategy', labelpad=60, fontsize=18, weight='bold')
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, f"Fig4_3D_{rep_name}_V7.png"), dpi=300)
    plt.close()
...
# --- REVISION 3: Heatmap (Fig 6) with X Title and Normal Weight ---
print("Generating Fig 6 V7: Heatmap with X Title...")
...
# FIX: Removed weight='bold' from xticks, added X label
plt.xticks(rotation=0, fontsize=16) # Removed weight='bold'
plt.yticks(fontsize=14)
plt.xlabel("Method Combination", fontsize=18, weight='bold') # Added Title
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "Fig6_Heatmap_Top15.png"), dpi=300)
plt.close()

print("Final Polish V7 Complete.")
