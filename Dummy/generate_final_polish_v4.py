...
OUTPUT_DIR = r"e:\TESIS\01. Dokumen\03. Publikasi Komparasi\ga_method_comparison\final_polished_v4"

if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

print("Starting FINAL POLISH V4 Analysis...")
...
# --- REVISION 1: Boxplot per Class Group (With X Label) ---
print("Generating Fig 2 V4: Boxplots with X Label...")

def plot_class_boxplot_v4(class_prefix, filename_suffix):
    ...
    plt.figure(figsize=(20, 9)) # Slightly taller for x label
    ...
    plt.xlabel("Solomon Instance", fontsize=LABEL_FONT_SIZE, weight='bold', labelpad=15) # ADDED
...
plot_class_boxplot_v4(tuple(['C']), "C")
plot_class_boxplot_v4(tuple(['R']), "R")
plot_class_boxplot_v4(tuple(['RC']), "RC")


# --- REVISION 2: 3D Plot Layout Fix (Margins) ---
print("Generating Fig 4 V4: 3D Plots with Margins...")

def plot_3d_v4(rep_name):
    ...
    sizes = (1 - fit_norm) * 800 + 50 # Even bigger balls

    fig = plt.figure(figsize=(20, 16)) # Huge canvas
    ...
    ax.set_zticklabels([nice_label(x) for x in mut_order], fontsize=11, rotation=0, ha='left') # Force straight Z
    ax.tick_params(axis='z', pad=20) # Push Z labels out

    ax.set_xlabel('Selection Method', labelpad=45, fontsize=16, weight='bold') 
    ax.set_ylabel('Crossover Operator', labelpad=45, fontsize=16, weight='bold')
    ax.set_zlabel('Mutation Strategy', labelpad=30, fontsize=16, weight='bold')
    
    ax.view_init(elev=25, azim=-55) # Tweak angle slightly

    # Colorbar
    cbar = plt.colorbar(sc, shrink=0.6, aspect=20, pad=0.1)
    cbar.set_label('Average Total Distance', fontsize=14, weight='bold')
    cbar.ax.tick_params(labelsize=12)

    # Manual Margin Adjustment prevents clipping
    plt.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05)

    plt.savefig(os.path.join(OUTPUT_DIR, f"Fig4_3D_{rep_name}_V4.png"), dpi=300)
    plt.close()
...
print("Final Polish V4 Complete using manual margins.")
